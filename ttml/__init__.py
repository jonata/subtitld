...
def parse_ttml_file(filename):
    ...
    def render_subtitles(elem, timestamp, parent_style={}):
        ...
        result = re.sub(
            r'(?P<all>(\s|^)<(font color="([^"]+)"|i)>) +', '\g<all>', result)
    ...
    for i, (timestamp, content) in enumerate(rendered_grouped[:-1]):
        if content == '':
            continue
        subtitle = []
        subtitle.append(timestamp.total_seconds())
        subtitle.append(rendered_grouped[i+1][0].total_seconds() - timestamp.total_seconds())
